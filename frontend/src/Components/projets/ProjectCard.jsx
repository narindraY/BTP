...
const STATUS_CFG = {
  "En cours":   { bg: "bg-[var(--secondary)]/10", text: "text-[var(--secondary)]" },
  "Terminé":    { bg: "bg-[var(--hoover)]/15",    text: "text-[var(--primary)]"   },
  "En attente": { bg: "bg-[var(--primary)]/8",    text: "text-[var(--primary)]"   },
};
...
const ProjectCard = ({ project, onClick }) => {
  ...
  const statusCfg = STATUS_CFG[status] ?? { bg: "bg-[var(--primary)]/8", text: "text-[var(--primary)]" };
  const typeColor = TYPE_COLORS[type_projet] ?? "var(--primary)";
  const typeBg    = TYPE_BG[typeColor];
    ...
    >
      ...
      <div className="flex flex-col gap-4 p-4 flex-1">
        ...
        <div className="flex items-start justify-between gap-2">
          ...
          <span className={`shrink-0 text-[11px] font-semibold px-2.5 py-0.5 rounded-full ${statusCfg.bg} ${statusCfg.text}`}>
            {status ?? "—"}
          </span>
        </div>
      ...
      <div
        ...
        style={{ borderTop: "0.5px solid rgba(6,11,39,0.07)", color: "var(--secondary)", background: "rgba(12,122,196,0.03)" }}
      >
